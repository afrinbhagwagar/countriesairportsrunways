'use strict'
var module = angular.module('accenture2.controllers', []);
module.controller("accenturecontroller2", [ "$scope", "accentureservice",
		function($scope, accentureservice) {
		accentureservice.getRunwaysByCountry(countryName).then(function() {
		accentureservice.getRunwaysByCountry(countryName).then(function(value) {
			...
			console.log("Error occured when getting all runways by country.");
		});
	}, function(reason) {
		console.log("Error occured when getting all runways by country.");
	});
